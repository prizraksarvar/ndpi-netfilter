...
u_int16_t ndpi_guess_host_protocol_id(struct ndpi_detection_module_struct *ndpi_str,
  ...
  if(ndpi_str && flow && flow->packet.iph) {
    struct in_addr addr;
    ...
    if((flow->l4_proto == IPPROTO_TCP) && flow->packet.tcp) {
        sport = flow->packet.tcp->source;
        dport = flow->packet.tcp->dest;
    } else if((flow->l4_proto == IPPROTO_UDP) && flow->packet.udp) {
        sport = flow->packet.udp->source;
        dport = flow->packet.udp->dest;
    } else {
        sport = dport = 0;
    }
    ...
    if(ret == NDPI_PROTOCOL_UNKNOWN && flow && flow->packet.iph) {
